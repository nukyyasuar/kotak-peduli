...
function NavBtn({ href, text, variant = "nav" }) {
  ...
  const baseClasses = "rounded-lg flex items-center justify-center transition";

  const variants = {
    nav: "py-1 rounded-none hover:border-b-3 hover:border-[#543a14] hover:font-bold hover:text-[#543a14]",
    btn: "border-2 border-[#F0BB78] text-[#131010] font-bold hover:bg-[#F0BB78] hover:text-white px-7 h-10",
...
const LogoutMenu = ({ handleLogout, className }) => {
  return (
    <div className={`absolute w-10 h-12 py-2 text-black ${className}`}>
