...
export const loginWithEmail = async (email, password) => {
  try {
    ...
    const response = await fetch(
      "https://clear-mammal-simply.ngrok-free.app/auth/login",
      requestOptions
    );
    ...
    if (!response.ok) {
      throw new Error(
        result.meta?.message?.join(", ") || result.message || "Login failed"
      );
    }
...
export const registerWithEmail = async ({
  ...
  try {
    ...
    const response = await fetch(
      "https://clear-mammal-simply.ngrok-free.app/auth/register",
      requestOptions
    );
    const result = await response.json();
